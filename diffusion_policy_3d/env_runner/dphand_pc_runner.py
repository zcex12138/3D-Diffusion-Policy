...
class DphandPointCloudRunner(BaseRunner):
    def __init__(self,
                 output_dir,
                 eval_episodes=20,
                 max_steps=500,
        ...
        def env_fn(): 
            cfg_path = Path(__file__).parent.parent / "env" / "dphand" / "configs" / "pick_and_place_pc.yaml"
            env = make_env(
                str(cfg_path),
                render_mode=render_mode,
                use_tactile_obs=use_tactile_obs,  # if use tactile obs
                wrapper_overrides={
                    'TeleopIntervention': {
                        'enabled': False,
                    }
        ...
        self.use_object_pos = use_object_pos
