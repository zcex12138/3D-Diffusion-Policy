...
class DphandPickCubeEnv(MujocoGymEnv):
    ...
    def __init__(
        ...
        self.cam_names = ["input", "fixed_cam"]
        self.cam_ids = dict({
    ...
    def reset(
        ...
        self.data.jnt("block").qpos[:3] = [0, 0.2, 0.05] + np.random.uniform(-0.02, 0.02, size=3)
        self.data.jnt("block2").qpos[:3] = [0.3, 0.2, 0.05] + np.random.uniform(-0.04, 0.04, size=3)
    ...
    def _compute_observation(self) -> dict:
        ...
        if self.image_obs:
            ...
            obs["images"]["fixed_cam"] = self._viewer.render_rgb_cam("rgb_array", self.cam_ids["fixed_cam"])
